...
@EnableWebSecurity // 시큐리티라는 필터가 등록이 된다.= 이미 활성화된 스프링 시큐리티가 어떤 설정을 해당 파일(SecurityConfig)에서 하겠다 
...
public class SecurityConfig{
	 ...
	 * Exception { //패스워드 비교를 못해서 꼭 이걸 만들어줘야해
	 * auth.userDetailsService(principalDetailService).passwordEncoder(encodePWD());
	 * //222.principalDetailService가 로그인 요청을 하고 리턴이 되면 passwordEncoder로 해서 사용자가 적은
	 * 패스워드를 encodePWD()로 암호화 후 DB랑 비교 //비교가 끝나서 다 맞으면 스프링 시큐리티영역에 우리 유저 정보가
	 * principalDetail로 감싸져서 저장이 된다 }
	 */
